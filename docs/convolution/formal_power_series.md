...
| `composition(f, g, deg)` / `composition(f, g)` | 合成 $f(g(x)) \bmod x^{deg}$ | — | $O(deg \log^2 deg)$ |
| `compositional_inverse(f, deg)` / `compositional_inverse(f)` | 合成逆 ($f(g) = g(f) = x \bmod x^{deg}$) | `f[0] == 0`, `f[1] != 0` | $O(deg \log^2 deg)$ |
