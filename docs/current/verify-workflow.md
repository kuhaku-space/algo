...
`schedule` は、push の無い週でもキャッシュを温存するための保険であり、同時に外部ジャッジ
（AOJ など）に対する定期的な再チェックでもある。

**週 1 回では間に合わない。** `schedule` の起動は高負荷時に数十分〜数時間遅れるので、間隔を
7 日ちょうどにすると遅延がそのまま退避期限の超過になる。実際に 18:00 指定の実行が 20:31 に
起動して前回から 7.09 日空き、問題キャッシュが退避されてテストケースの再ダウンロードで
32 分（通常は 3〜4 分）かかったうえ、cold cache 由来の AOJ タイムアウトで失敗した例がある。

- **週 2 回（日・水）**にして最大間隔を 4 日にし、遅延に対する余裕を作る。
- **毎時 0 分を避ける**（`35` 分にしてある）。毎時 0 分は最も混み、遅延が大きくなる。
- キャッシュが温かい定期実行は 3〜4 分で終わるので、回数を増やすコストはほぼ無い。
