...
struct Graph {
    ...
    Graph(int64_t v) : V(v), edges(v * 5), dist(v, numeric_limits<T>::max()) {
        for (int64_t i = 2; i < v * 2; ++i) {
    ...
    void add_edge(int64_t a1, int64_t b1, int64_t a2, int64_t b2, T d = 1,
                  bool is_dual = false) {
        ...
        for (a1 += V, b1 += V; a1 < b1; a1 >>= 1, b1 >>= 1) {
            ...
            if (b1 & 1)
                --b1, edges[V * 3 + b1].emplace_back(edge{V * 3 + b1, n, 0});
        }
