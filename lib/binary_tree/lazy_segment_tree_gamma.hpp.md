...
data:
  ...
  code: "#include \"template/template.hpp\"\r\n\r\ntemplate <class T, class U, class\
    ...
    \   const T e;\r\n    const U id;\r\n    const F op;    // (T, T) -> T\r\n   \
    \ const G mapp;  // (U, T) -> T\r\n    const H comp;  // (U, U) -> U\r\n    vector<T>\
    \ data;\r\n    vector<U> lazy;\r\n\r\n    lazy_segment_tree(int _n, const T &_e,\
    \ const U &_id, const F &_op,\r\n                      const G &_mapp, const H\
    \ &_comp)\r\n        : e(_e), id(_id), op(_op), mapp(_mapp), comp(_comp) {\r\n\
    \        init(_n);\r\n    }\r\n\r\n    void init(int n) {\r\n        for (N =\
    \ 1, logN = 0; N < n; N <<= 1, ++logN)\r\n            ;\r\n        data.assign(N\
    \ << 1, e);\r\n        lazy.assign(N << 1, id);\r\n    }\r\n\r\n    template <class\
    \ V>\r\n    void build(const vector<V> &v) {\r\n        for (int i = 0; i < v.size();\
    \ ++i) data[N + i] = v[i];\r\n        for (int i = N - 1; i >= 1; --i)\r\n   \
    \         data[i] = op(data[i * 2], data[i * 2 + 1]);\r\n    }\r\n\r\n    void\
    \ apply(int k, const U &x) {\r\n        assert(k > 0 && k < N << 1);\r\n     \
  ...
  verifiedWith: []
documentation_of: lib/binary_tree/lazy_segment_tree_gamma.hpp
layout: document
redirect_from:
- /library/lib/binary_tree/lazy_segment_tree_gamma.hpp
- /library/lib/binary_tree/lazy_segment_tree_gamma.hpp.html
