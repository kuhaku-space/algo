...
data:
  ...
  code: "#include \"template/template.hpp\"\r\n\r\nnamespace FFT_MOD {\r\nusing CP\
    ...
    \ N = a.size();\r\n    static bool is_first = true;\r\n    static vector<CP> vbw(30),\
    \ vibw(30);\r\n    if (is_first) {\r\n        is_first = false;\r\n        for\
    \ (int i = 0; i < 30; ++i) {\r\n            vbw[i] = polar(1.0, 2.0 * PI / (1\
    \ << (i + 1)));\r\n            vibw[i] = polar(1.0, -2.0 * PI / (1 << (i + 1)));\r\
    \n        }\r\n    }\r\n    for (int i = 0, j = 1; j < N - 1; ++j) {\r\n     \
    \   for (int k = N >> 1; k > (i ^= k); k >>= 1)\r\n            ;\r\n        if\
    \ (i > j) swap(a[i], a[j]);\r\n    }\r\n    for (int k = 0, t = 1; t < N; ++k,\
    \ t <<= 1) {\r\n        CP bw = vbw[k];\r\n        if (inv) bw = vibw[k];\r\n\
    ...
    \ va[i] = CP(a[i]);\r\n    for (int i = 0; i < b.size(); ++i) vb[i] = CP(b[i]);\r\
    \n    _fft(va, false), _fft(vb, false);\r\n    va *= vb;\r\n    _fft(va, true);\r\
    \n\r\n    a.resize(n);\r\n    for (int i = 0; i < n; ++i) a[i] = T(va[i].real()\
    \ + 0.5);\r\n}\r\n\r\ntemplate <class T>\r\nvalarray<T> _convolution(const valarray<T>\
    \ &a, const valarray<T> &b) {\r\n    valarray<T> res = a;\r\n    _convolution_self(res,\
