...
/// @brief 2 彩色済みの一般の無向グラフを Hopcroft-Karp に橋渡しするアダプタ
...
///              auto mis = bm.maximum_independent_set();
///          }
///          ```
struct BipartiteMatching {
    ...
    BipartiteMatching(const G &g, const std::vector<int> &color)
        : local(color.size(), -1), hk((int)std::count(color.begin(), color.end(), 0),
        ...
        for (int v : origin_left) {
            for (auto &e : g[v]) {
                assert(color[e.to()] == 1);
                hk.add_edge(local[v], local[e.to()]);
            }
        }
    }
    ...
    /// @brief 最大マッチングのサイズ
    int matching() {
        if (match_size == -1) match_size = hk.matching();
        return match_size;
    }
