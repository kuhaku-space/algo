...
///   入力中の括弧 `()` は固定のグループ化として扱い、その内側も再帰的に
///   全順序を試す。`op(c, f)` で演算子を登録する。`f` は無効な演算
///   （$0$ 除算など）に対し `std::nullopt` を返すとその経路を捨てる。
///   値の集合を返すので、`size()` で「あり得る結果の通り数」が得られる
///   (AOJ 2255 など)。オペランド数を $k$ とすると区間 DP で
///   $O(k^2)$ 個の部分区間 $\times$ 集合サイズの二乗で評価する。
