...
/// @details `(tag, 子 id の列)` の組を一意な整数 id に写す。構造的に等しい
...
///   などに使う。
/// @tparam Tag ノードのラベル型。`std::hash` と `==` を持つ型
///   （`int`・`char`・`std::string` 等）。
/// @complexity ノードの登録は子数を $k$ として期待 $O(k)$、参照は $O(1)$
template <class Tag = int>
struct HashCons {
    /// @brief ノード `(tag, children)` をインターンして id を返す
    /// @details 同じ `(tag, children)` に対しては常に同じ id を返す。
    ///   `children` は各要素が既存の id（`[0, size())` の範囲）であること。
