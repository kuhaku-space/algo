...
struct persistent_segment_tree {
  ...
  public:
    ...
    persistent_segment_tree(int n, T e = Monoid::id) : _size(n), root(build(0, n, e)) {}
    template <class U>
    ...
    persistent_segment_tree set(int k, T val) {
        assert(0 <= k && k < _size);
        return persistent_segment_tree(_size, set(0, _size, k, val, root));
    }
