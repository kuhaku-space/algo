...
data:
  ...
  code: "#pragma once\n#include \"template/template.hpp\"\nusing ll = int64_t;\nusing\
    ...
    \ n) for(int64_t i = (m); i < int64_t(n); ++i)\n#define rep(i, n) FOR(i, 0, n)\n\
    #define repn(i, n) FOR(i, 1, n+1)\n#define repr(i, n) FORR(i, n, 0)\n#define repnr(i,\
    \ n) FORR(i, n+1, 1)\n#define all(s) (s).begin(), (s).end()\ntemplate<class T,\
    \ class U>\nstd::istream &operator>>(std::istream &is, std::pair<T, U> &p) { is\
    \ >> p.first >> p.second; return is; }\ntemplate <class T>\nstd::istream &operator>>(std::istream\
    \ &is, std::vector<T> &v) { for (T &i : v) is>>i; return is; }\ntemplate <class\
    ...
    \ is_correct=true) { std::cout<<(is_correct?\"YES\":\"NO\")<<std::endl; }\nvoid\
    \ NO(bool is_not_correct=true) { YES(!is_not_correct); }\nvoid Takahashi(bool\
    \ is_correct=true) { std::cout<<(is_correct?\"Takahashi\":\"Aoki\")<<std::endl;\
