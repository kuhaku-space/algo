---
data:
  _extendedDependsOn:
  - icon: ':question:'
    path: lib/template/template.hpp
    title: lib/template/template.hpp
  - icon: ':heavy_check_mark:'
    path: lib/tree/partially_persistent_union_find.hpp
    title: "\u90E8\u5206\u6C38\u7D9AUnion-Find"
  _extendedRequiredBy: []
  _extendedVerifiedWith: []
  _isVerificationFailed: false
  _pathExtension: cpp
  _verificationStatusIcon: ':heavy_check_mark:'
  attributes:
    '*NOT_SPECIAL_COMMENTS*': ''
    PROBLEM: https://yukicoder.me/problems/no/416
    links:
    - https://yukicoder.me/problems/no/416
  bundledCode: "Traceback (most recent call last):\n  File \"/opt/hostedtoolcache/Python/3.10.13/x64/lib/python3.10/site-packages/onlinejudge_verify/documentation/build.py\"\
    , line 71, in _render_source_code_stat\n    bundled_code = language.bundle(stat.path,\
    \ basedir=basedir, options={'include_paths': [basedir]}).decode()\n  File \"/opt/hostedtoolcache/Python/3.10.13/x64/lib/python3.10/site-packages/onlinejudge_verify/languages/cplusplus.py\"\
    , line 187, in bundle\n    bundler.update(path)\n  File \"/opt/hostedtoolcache/Python/3.10.13/x64/lib/python3.10/site-packages/onlinejudge_verify/languages/cplusplus_bundle.py\"\
    , line 401, in update\n    self.update(self._resolve(pathlib.Path(included), included_from=path))\n\
    \  File \"/opt/hostedtoolcache/Python/3.10.13/x64/lib/python3.10/site-packages/onlinejudge_verify/languages/cplusplus_bundle.py\"\
    , line 260, in _resolve\n    raise BundleErrorAt(path, -1, \"no such header\"\
    )\nonlinejudge_verify.languages.cplusplus_bundle.BundleErrorAt: tree/partially_persistent_union_find.hpp:\
    \ line -1: no such header\n"
  code: "#define PROBLEM \"https://yukicoder.me/problems/no/416\"\n#include <algorithm>\n\
    #include <iostream>\n#include <utility>\n#include <vector>\n#include \"tree/partially_persistent_union_find.hpp\"\
    \n\nint main(void) {\n    int n, m, q;\n    std::cin >> n >> m >> q;\n    std::vector<std::pair<int,\
    \ int>> es(m);\n    for (auto &[a, b] : es) std::cin >> a >> b;\n    std::vector<std::pair<int,\
    \ int>> queries(q);\n    for (auto &[a, b] : queries) std::cin >> a >> b;\n  \
    \  std::reverse(queries.begin(), queries.end());\n    std::set<std::pair<int,\
    \ int>> st(queries.begin(), queries.end());\n\n    partially_persistent_union_find\
    \ uf(n);\n    for (auto [a, b] : es) {\n        if (!st.count({a, b})) uf.unite(a\
    \ - 1, b - 1);\n    }\n    for (auto [a, b] : queries) uf.unite(a - 1, b - 1);\n\
    \n    for (int i = 1; i < n; ++i) {\n        if (!uf.same(0, i)) {\n         \
    \   std::cout << 0 << '\\n';\n            continue;\n        }\n        if (uf.same(0,\
    \ i, m - q)) {\n            std::cout << -1 << '\\n';\n            continue;\n\
    \        }\n        auto check = [&](int mid) { return !uf.same(0, i, m - mid);\
    \ };\n        int l = q, r = -1;\n        while (abs(l - r) > 1) {\n         \
    \   int mid = (l + r) / 2;\n            (check(mid) ? l : r) = mid;\n        }\n\
    \        std::cout << l << '\\n';\n    }\n\n    return 0;\n}\n"
  dependsOn:
  - lib/tree/partially_persistent_union_find.hpp
  - lib/template/template.hpp
  isVerificationFile: true
  path: test/yukicoder/0416.test.cpp
  requiredBy: []
  timestamp: '2023-11-15 19:48:52+09:00'
  verificationStatus: TEST_ACCEPTED
  verifiedWith: []
documentation_of: test/yukicoder/0416.test.cpp
layout: document
redirect_from:
- /verify/test/yukicoder/0416.test.cpp
- /verify/test/yukicoder/0416.test.cpp.html
title: test/yukicoder/0416.test.cpp
---
